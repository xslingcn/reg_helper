...
pub(crate) async fn match_section(sln: &str) -> Option<(&String, &Section)> {
    for (name, section) in CONFIG.sections.iter() {
...
pub(crate) async fn register(sln: &str) -> RegResult<String> {
    ...
    if let Some((name, switch)) = switch::match_switch(sln) {
        println!("Switching for {}", name);
        return switch::switch(switch).await;
    }

    let url;
    if let Some((name, section)) = match_section(sln).await {
        url = format!(
...
pub(crate) fn parse_status(res: &str) -> RegResult<String> {
